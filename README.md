...
防災力を高める様々な仕組みをオンライン・プラットフォームとして構築します。これはオープンイノベーション、リビングラボなど各ステークホルダーと連携を可能とし、さらにはSDGsを考慮した取り組みを積極的に推進します。
   1. 3つの取り組み
   新しい取り組みを創造してみました。(1)オンライン防災、(2)オンライン防災拠点の開設、(3)避難シミュレーションの実施、(4)「我が家の防災マニュアル」作成ワークショップの実施、(5)避難所のパンク防止と自助推進、以上の5つです。
 
   2. 防災設備（防災パーゴラ等）の設置
   (1)耐震設計したパーゴラを設置する、(2)パーゴラにテントを張り災害時に避難所として使用する、(3)貯水設備を設置（発災後に使用されることを前提としている）して利用できる様にする、以上の4つです。
...
## ハイブリッド型オープンイノベーションと市民活動
   ...
   3. 社会課題解決及び政策提言を協同できる仕組みを複数のフェーズに基づいて実行します。(1)Mining（発掘する）：様々な観点から課題を発掘、(2)Work（提案づくり）：発見された課題からの提案づくり、(3)Do（提案する）：提案づくりしたものを関係先に提出、(4)Check（検証する）：提出された提案の検証、⑤Act（改善する）：検証した提案の改善
...
アルコロジー（Arcology: The City in the Image of Man）とは、アメリカの建築家であるパオロ・ソレリ（en:Paolo Soleri）が創り出した建築 (architecture) と生態学 (ecology) から成る混成語であり、「ニュータウン・アルコロジー」とは、ニュータウンを1つ空間に位置づけ、環境を守りつつ、高密度化した自己完結都市を目指す考えであり、それに関する構想です。
   ...
   3. 新規まちづくりNPO設立支援制度
これは「NPOのためのNPOが必要である」（南美希）という指摘に基づき、新規で街づくりNPO団体を設立するときに、円滑化を図る目的で支援する制度です。これに際しては、担当アドバイザーの設置を行政に求めるものです。
